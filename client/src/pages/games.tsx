...
export default function Games() {
  ...
  const { data: contentData, isLoading } = useQuery({
    queryKey: ['/api/content/games', filters],
    queryFn: async () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      {/* Header */}
      <div className="bg-gradient-to-r from-green-900 to-blue-900 py-8">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl font-bold text-center mb-4">الألعاب</h1>
          <p className="text-center text-white/80 max-w-2xl mx-auto">
            اكتشف أحدث الألعاب الإلكترونية والتطبيقات الترفيهية
          </p>
        </div>
      </div>
