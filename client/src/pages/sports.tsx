...
export default function Sports() {
  ...
  const { data: contentData, isLoading } = useQuery({
    queryKey: ['/api/content/sports', filters],
    queryFn: async () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      {/* Header */}
      <div className="bg-gradient-to-r from-teal-900 to-green-900 py-8">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl font-bold text-center mb-4">الرياضة</h1>
          <p className="text-center text-white/80 max-w-2xl mx-auto">
            اكتشف أحدث الأحداث الرياضية والمباريات العالمية
          </p>
        </div>
      </div>
